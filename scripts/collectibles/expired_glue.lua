...
function ExpiredGlue:PostPickupUpdate(pickup)
    ...
    if PlayerManager.AnyoneHasCollectible(ty.CustomCollectibles.EXPIREDGLUE) and not PlayerManager.AnyoneHasCollectible(ty.CustomCollectibles.CURSEDTREASURE) and pickup.FrameCount % 6 == 0 and not pickup:GetSprite():IsPlaying("Collect") then
