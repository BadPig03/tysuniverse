...
function CrownOfKings:PreSpawnCleanAward(rng, spawnPosition)
    ...
    for _, player in pairs(PlayerManager.GetPlayers()) do
        ...
        if player:HasCollectible(ty.CustomCollectibles.CROWNOFKINGS) and data.CrownOfKings.CanSpawn then
            ...
            Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, collectibleID, room:FindFreePickupSpawnPosition(spawnPosition + Vector(0, 40), 0, true), Vector(0, 0), nil) 
            data.CrownOfKings.CanSpawn = false
            player:AnimateHappy()
        end
    end
